...
def recom_ranked(original_df, recommended_dataframe, barcode):
	...
	# Cosine similarity
	df_ranked = cosine(recommended_dataframe)
	df_ranked['cos_total'] = (df_ranked['cos_product_name_clean'] + df_ranked['cos_ingredients_clean']).round(2)
	df_ranked_sorted = df_ranked.sort_values(['cos_total'], ascending=[False]).reset_index()
	...
	sorterIndex = dict(zip(df_rules_list,range(len(df_rules_list))))
	df_ranked_sorted['Rank'] = df_ranked_sorted['product_name_clean'].map(sorterIndex)
	df_ranked_sorted.sort_values(['cos_total', 'Rank'], ascending = [False, True], inplace = True)
...
def eligible_swaps(barcode, df):
	...
	elif (df[df['barcode'] == barcode]['badge_new'] == 1).all():
			...
			if len(df_swaps) == 0:
				...
				if len(new_df_pgc) == 0:
					...
					while len(new_df_pgc) <3:
						...
						tries += 1
						if tries >= 15:
							raise Exception('Sorry, this product does not have enough swaps to build a top 3.') 
...
def top_100_swaps(barcode, df):
	...
	# full_100.sort_values(['cos_total'], ascending=False, inplace=True)
...
def selection(barcode, df):
	...
	'''Function to filter DataFrame for manufacturer, PHE_cat and product according to the scanned barcode.
	The function also defines ranges for packsize and packcount based on
	the desired user-defined size percent and count percent.'''
	...
	if packsize==0 and PHE_cat is not None:
		...
		df_manuf = df[df.manufacturer_clean.isin(manufacturer)&
					(df.PHE_cat==PHE_cat)&
					(packcount_low<=df.packcount.astype(int))&
					(df.packcount.astype(int)<=packcount_high)]

		df_PHE_cat = df[(df.PHE_cat==PHE_cat)&
							 (df.barcode!=barcode)&
		...
		df_PHE_cat = df[(df.category==category)&
								(df.barcode!=barcode)&
								~ (df.manufacturer_clean.isin(manufacturer))&
								(packsize_low<=df.packsize.astype(int))&
								(df.packsize.astype(int)<=packsize_high)&
...
def recom_check(barcode, df):
	...
	if (barcode in df_check_final.barcode.values) == False:
	...
	if len(df_check_final) < n_recommendation:
		...
		# run through the selection check again but with subcategories added to get more eligible swaps
		manufacturer=df.loc[df['barcode']==barcode, 'manufacturer_clean']
		...
		if packsize==0:
			...
			df_PHE_cat = df[(df['main_and_sub1']==PHE_cat)&
						   (df.barcode!=barcode)&
						   ~ (df.manufacturer_clean.isin(manufacturer))&
						   (packcount_low<=df.packcount.astype(int))&
						   (df.packcount.astype(int)<=packcount_high)]
		...
		df_check_final = df_check_final[['barcode','product_name','product_name_clean','ingredients_clean',
										'badge_new', 'packsize', 'packcount', 'sugar_100', 'fat_100', 'salt_100',  
										'main_category_name', 'sub_category_name', 'product_category_name', 
										'high_five_man','packsize_extract','PHE_cat', 'manufacturer_clean', 
										'retailer_extract', 'pgc_badge']]
	else:
	...
	df_check_final['ingredients_clean'].fillna(df_check_final['product_name_clean'], inplace=True)

	# Filter by same retailer and null values
	retailer = df[df['barcode'] == barcode]['retailer_extract']
	retailer = retailer.to_list()
	df_check_final = df_check_final[(df_check_final['retailer_extract'].isin(retailer))|
...
def cosine(df):
	...
	for i in columns_list:
		...
		df_model = pd.DataFrame(doc_term_matrix,
